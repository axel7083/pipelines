...
def _resolve_model(
    executor_input,
    gcp_resources,
    model_artifact_resource_name: Optional[str] = None,
    model_name: Optional[str] = None,
    model_version: Optional[str] = None,
):
  """Resolve model."""
  if model_version:
    model_version_suffix = f'@{model_version}'
  else:
    model_version_suffix = ''

  if model_name:
    model_resource_name = f'{model_name.split("@", 1)[0]}{model_version_suffix}'
  elif model_artifact_resource_name and model_version_suffix:
    model_resource_name = (
        f'{model_artifact_resource_name.split("@", 1)[0]}{model_version_suffix}'
    )
  else:
    model_resource_name = model_artifact_resource_name
...
def main(argv):
  ...
  Expected input args are as follows:
    ...
    model_artifact_resource_name - Required or model_name is required. The
        existing model artifacts resource name.
    model_name - Required or model_artifact_resource_name is required. Provided
        string resource name to create a model artifact.
    model_version - Optional. The models version, overrides any versions in
        model_artifact_resource_name or model_name.
